...
使用したコマンド  
insert into meibo(number,username) values(1,'Kawaguchi Ryota');  
...
row　=　行  
カラム　=　列  
query　= 英語では問い合わせという意味  
fetch(フェッチ) = 取ってくる  
assoc(アソック) = Associative array(連想配列)の略称 データに名前（キー）をつけて管理する形式  

$row = $result->fetch_assoc();　を実行すると、$rowの中身が以下のようになる  
・$row["number"] →１  
・$row["username"] → Kawaguchi Ryota  

補足：assocがなかった場合（つまり、データに名前をつけず番号で管理する方式）  
fetch_row()と記載することになるが  
これだとこの後のコードに$row[0]や$row[1]と書く必要が生じる。  

もちろん数字で管理しても良いが、$row["number"]や$row["username"]と記述して  
データを取り出せた方が、直感的に操作することができる  

=結論、assocを記述してデータを管理した方が良い  
参考にしたWebサイト(https://www.php.net/manual/ja/function.mysql-fetch-assoc.php)  

### 実際に表示してみる  
ここまでコードを書いて、かつアクセス出来る権限も設定したので恐らくこれで無事にWebサイトを表示できると  
思いましたが、phpの学習をしている時にphpをmysqlと結びつけるパッケージをインストールする記載を  
無視して進めていたので(mysqlの課題と並行してインストールする為)、このタイミングでインストールしてからページを表示してみることにする  

使用したコマンド  
sudo apt install php7.4-mysql  
sudo systemctl restart apache2  

参考にしたWebサイト(https://qiita.com/yuu1111main/items/98baacaf381a2b778ccc)  

その後課題の【http://<自分の名前>-hbtask.local/mysql.php`にアクセス】を実行して表示できたのだが  
<img width="1087" height="597" alt="image" src="https://github.com/user-attachments/assets/de496a3d-dd08-412e-b0e8-d726448d8617" />  

ただ、課題にある<自分の名前>を<自分の名字>で作成してしまっていた為、課題に沿うように修正  

①sudo vi /var/www/kawaguchi-hbtask.local/mysql.phpの内容をコピーし-ryota-hbtask.local-(略称)の中に貼り付ける  
②-ryota-hbtask.local-でページが表示されることを確認し、-kawaguchi-hbtask.local-の中身を全て削除  
　補足：全て削除のviコマンド 【:%d】  

課題の最終盤  
<img width="999" height="383" alt="image" src="https://github.com/user-attachments/assets/b95c0b40-0b15-4a27-b27e-cb39b444b995" />  

## ５．次回やってみたいこと  
・データベースが２つ以上ある時の使い分け  
・また、一つのページ(ブラウザ)に対し複数のデータベースからそれぞれデータを抽出することは出来るのか検証  
